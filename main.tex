...
The pooled recombination map cannot resolve the inversion. The estimated rate inside the inverted body equals that of the collinear flanks (medians 0.2015 and 0.2023~cM/Mb), and chromosome~2 as a whole is an ordinary autosome (median 0.202 against an autosomal median of 0.200, the 60th percentile of per-chromosome medians; Supplementary Fig.~S12). This map was estimated from all karyotypes together and from linkage disequilibrium within $\sim$19-kb windows, whereas the barrier acts only between arrangements and at the scale of the whole 19-Mb block. Within each window the many SNPs that segregate within arrangements dominate the signal, and the between-arrangement association carried by the few near-fixed differences is spread across megabases. The barrier is instead shown directly by the linkage-disequilibrium block that spans the inversion, by the arrangement differentiation, and by the joint site-frequency spectrum (\cref{fig:inv}b; Supplementary Figs.~S4, S5). It is not locally adaptive, since there is
