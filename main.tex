...
largest effective population sizes of any animal --- was recently shown to be panmictic across its
northwest Atlantic range \citep{baker2025}. Using whole-genome data from 633 individuals across that range,
we show that this unstructured genome carries a single $\sim$19~Mb inversion on chromosome~2, resolve what
the inversion is, and map the genome-wide landscape of recent positive selection around it. The inversion
...
show extensive lineage-specific reorganisation \citep{schmidbaur2022} and pervasive RNA-editing-based
transcriptome plasticity \citep{liscovitch2017},
and their photoreception extends beyond the camera eye to dermal opsins and light-driven chromatophore
control \citep{mathger2010,ramirez2015,kingston2015}. The northern shortfin squid \illex{} is an annual,
...
A recent population-genomic survey of \illex{} \citep{baker2025} found no genetic structure across more
than 2{,}500~km of range and no evidence of large structural variants, and recommended management as a
single stock. We confirm the panmixia here, but find beneath it a single, striking axis of structure on
chromosome~2 --- recovered by PCA, $\Fst$, and windowed local PCA and attributable to a large inversion ---
that raises the questions motivating this study. First, \emph{what is} the chromosome-2 inversion --- a
...
enforces the cross-karyotype recombination barrier, tracks ancestral material with a hull-based scheme in
the msprime lineage \citep{kelleher2016,baumdicker2022}, and outputs tree sequences (\cref{sec:methods}). The data constrain the \emph{shape} of
...
load accumulates \citep{zhao2016,gilbert2020,berdan2021}, and the clustered coding divergence we document --- 19 exonic
fixed differences in nine genes, in a block whose 45 annotated genes show no functional enrichment --- is the
raw material of such a load. But a two-phase history of plateau followed by decline is also what selection toward a shifting
...
\paragraph{Data.}
The ingroup is the 633-individual \illex{} whole-genome callset from the sequencing programme described by
\citet{baker2025}, mapped to the
...
\begin{figure}[p]\centering
  ...
  \caption{\textbf{No population structure outside chr2, and the sex assignment} (consistent with the
    range-wide panmixia of \citealt{baker2025}). \textbf{(a)} Genome-wide autosomal PCA with chr2 excluded --- a single
