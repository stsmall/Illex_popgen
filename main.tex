...
complete range-wide panmixia, but identify a single major axis of genomic structure, a $\sim$19~Mb inversion on chromosome~2. The derived arrangement segregates at a
...
cross-method-corroborated scan for recent positive selection identified 627 candidate sweep windows, including 24 high-confidence regions independently supported by
multiple methods. Sweep-associated genes show strong and background-selection-robust
enrichment for two biological themes, neurodevelopment and photoreception, with additional candidates involving lineage-specific genes and Z-linked RNA-regulatory
and hormone-signalling loci. Together, these results reveal two contrasting modes of
adaptation in an otherwise panmictic marine species, namely a long-lived chromosomal polymorphism maintained by balancing selection and recent directional selection on
...
Marine populations with enormous census sizes and high dispersal present a persistent problem for population genetics, because weak geographic differentiation can coexist with
...
These analyses resolve two distinct forms of genomic adaptation in an otherwise panmictic species, a long-lived, balancing-selected
...
explains none of the variation on any other chromosome (in a per-chromosome PCA coloured by karyotype, $\eta^{2}=0.76$ on chr2 versus $\leq0.01$ on all 45 others). Second, and independently of the chr2 labels,
...
genotype clusters on chromosome~2 (254 AA, 284 AB, and 95 BB individuals; \cref{fig:inv}a) whose differentiation is karyotypic rather than geographic, with $\Fst(\mathrm{AA,BB})=0.37$ inside the inversion against a
...
lines pin the breakpoints to chr2:60.54--79.50~Mb (a $\sim$18.96~Mb differentiated body), namely a step-change in arrangement differentiation and a strand flip in a whole-chromosome alignment to \coin{}
...
alternate allele shows AA to be ancestral (63.0\%) and BB derived. There was a single inversion event, and the reference was assembled from an inverted individual (Supplementary Table~S3).
...
inflating rare variants \citep{eldon2006,hedgecock2011,eldon2015}, we tested the coalescent class directly. The folded SFS is fit far better by a Kingman-plus-growth model than by any multiple-merger (Beta) coalescent
...
Three further features of the genome set the stage for the sweep scan. Recombination, as estimated by a deep-learning LD-based map, is $\sim$0.20~cM/Mb on the autosomes (median 0.200; Supplementary Fig.~S12). Because the estimator reads linkage disequilibrium it recovers the sex-averaged rate, so heterochiasmy is not identifiable from it. The map is flat with respect to gene content, and the rate varies by only $\sim$6\% between windows adjacent to exons and windows $>$100~kb from any exon, unchanged when $\pi$ is held fixed (Supplementary Fig.~S17a), as a crossover rate should be. It is also compressed at chromosome scale. On chromosome~1 the rolling median spans just 0.17--0.23~cM/Mb, the pericentromeric flanks where
$\pi$ halves show no more than a 5\% dip, and the centromere itself (a 2.5-Mb block that is 80\% repeat and
almost entirely masked) carries no windows (Supplementary Fig.~S22). The pattern is genome-wide. Across all 43 scanned autosomes the chromosome-scale rolling median varies by only 10--41\% of each chromosome's
...
rather than by the recombination axis. Diversity is also heterogeneous at the megabase scale. Twenty-two blocks of $\geq$1~Mb on 17 chromosomes (129~Mb, 3.9\% of the autosomes) carry 2--6 times the chromosome-median $\pi$ with less negative Tajima's $D$ (Supplementary Table~S9). They are present at the same strength in every sampling division, are no more differentiated among divisions than flanking sequence, and lack the long-range linkage disequilibrium of a segregating inversion (Supplementary Figs.~S23, S24). Read depth resolves them into three kinds. In six, depth is raised in every individual (1.2--2.2 times the flanks across the population sample, and up to 2.7 and 2.4 times in long reads from the individual used for the assembly and a second individual), and a single diploid genome carries five to six times the flanking density of heterozygous sites at skewed allele fractions, the signature of duplicated sequence collapsed into one copy in the assembly. In eight, depth is reduced to 0.6--0.9 times the flanks in every individual, indicating haplotypes too divergent from the reference for many short reads to map; long reads, which span the divergent sequence, show a much smaller deficit. The remaining eight have normal depth and mapping and are gene-poor regions of genuinely elevated diversity. Summary statistics cannot separate any of these from the footprints of selection, so all 22 blocks were excluded from the sweep scan and the gene-set analyses. They hold 5\% of accessible sites, and excluding them lowers genome-wide $\pi$ by 9\% while shifting Tajima's $D$ by less than 0.02, within the uncertainty of the demographic fit. A model-based $B$-map could not be fit under this
