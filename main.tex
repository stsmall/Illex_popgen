...
rather than by the recombination axis. Diversity is also heterogeneous at the megabase scale, where 22 blocks of
$\geq$1~Mb on 17 chromosomes carry 2--4$\times$ the chromosome-median $\pi$ with less-negative Tajima's $D$
(e.g.\ chr1:24--31~Mb, $\pi=0.020$; Supplementary Fig.~S23, Table~S9). Every block is present at the same strength in each of the ten sampling divisions (block-to-flank ratio varying by 1--6\% among divisions), and between-division $\Fst$ is no higher inside the blocks than outside, so none is a regional or population-specific feature (Supplementary Fig.~S23). Nor do they carry the long-range linkage disequilibrium of a segregating inversion. The chromosome-2 inversion shows $r^2$ at $>$1~Mb 3.5 times that of its flanks, whereas every block tested is indistinguishable from flanking sequence (Supplementary Fig.~S24). We tested these against every structural
...
independently corroborated by a footprint method, namely the 34 cross-method-concordant regions (a diploS/HIC-HMM
outlier plus RAiSD or SweepFinder2). The two tiers agree strongly, with 89\% of windows at concordant loci are
...
yields 152 terms at FDR$<$0.05, and every leading term's fold enrichment rises, including response to light stimulus
