...
Three further features of the genome set the stage for the sweep scan. \emph{Recombination}, as estimated
by a deep-learning LD-based map built per sex, is $\sim$0.21--0.24~cM/Mb on the autosomes (sex-averaged
$\sim$0.244; Supplementary Fig.~S12); because the estimator reads linkage disequilibrium it recovers the
sex-averaged rate and heterochiasmy is not identifiable from it (the female-only map in fact collapses to a
near-constant and is uninformative). The map is flat with respect to gene content --- the rate varies by only
$\sim$6\% between windows adjacent to exons and windows $>$100~kb from any exon, unchanged when $\pi$ is held
fixed (Supplementary Fig.~S17a) --- as a crossover rate should be. It is also compressed at chromosome
...
\emph{Background selection} is moderate and gene-density-driven: $\pi$ falls with local coding density
($r=-0.31$; 20--30\% reduction in gene-dense windows) and rises 43\% between exon-adjacent and exon-distant
windows, while recombination does neither (Supplementary Fig.~S17) --- the classical footprint of linked
purifying selection \citep{charlesworth1993,mcvicker2009,charlesworth2021}, organised here by gene density
...
(e.g.\ chr1:24--31~Mb, $\pi=0.020$; Supplementary Fig.~S22c). We tested these against every structural
...
depth anomaly, and their allele-frequency spectrum is identical to flanking sequence --- they simply carry
$\sim$2.5$\times$ more SNPs. They are gene-poor (coding fraction 0.001), largely unalignable to \coin{}
...
fast-evolving, largely lineage-specific gene deserts --- elevated mutation input and relaxed linked selection
--- not structural polymorphisms, and the chromosome-2 inversion remains the genome's only segregating
...
over the per-window (100~kb) classes, with neutral, linked, and sweep-centre states whose emissions are the
...
it is used only for its genome-wide mean and for the decoder's dwell scaling, and the flatness of
diversity against recombination (Supplementary Fig.~S17d) should be read with that ceiling in mind. The
