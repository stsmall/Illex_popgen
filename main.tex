...
it is used only for its genome-wide mean and for the decoder's dwell scaling, and the absence of a
diversity--recombination relationship in these data should be read with that ceiling in mind. The
...
that study retained 540 after its own quality filters. We re-processed the raw reads from scratch: mapping to
...
\begin{figure}[p]\centering
  ...
  \caption{\textbf{The chromosome-2 inversion.} \textbf{(a)} chr2 PCA resolves three karyotype clusters
    ...
    collinear sequence on either side: inside the breakpoints the derived, inverted BB arrangement is
    \emph{more} diverse than the ancestral AA ($\pi_I/\pi_S=1.36$) despite its lower frequency, and $\dxy$
    exceeds both --- the signature of an old, formerly common polymorphism; outside them the three curves
...
\begin{figure}[p]\centering
  ...
  \caption{\textbf{The genome-wide selective-sweep scan.} Sweep signal across the genome (chromosomes
    ...
    Tier-1 calls (diamonds; \textit{Pasilla}, \textit{Smaug}, \textit{Esrrg} labelled) are male-based and
    diploS/HIC-only --- the footprint methods do not cover the Z --- so they cannot enter the concordance
...
\begin{figure}[p]\centering
  ...
  \caption{\textbf{Diversity is uniform and there is no geographic structure.} \textbf{(a)} Nucleotide
    ...
    3N, with a single individual, cannot support per-population estimates), ordered south$\to$north --- both are
