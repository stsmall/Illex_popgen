...
Outside the inversion, demographic analyses support strong expansion (Tajima's $D=-1.5$)
and an effective population size exceeding 4 million.A stratified,
...
are strongly influenced by transport in the Gulf Stream and associated shelf--slope
circulation, while adults undertake extensive horizontal and diel vertical migrations.
...
molluscs \citep{albertin2015,albertin2022,schmidbaur2022,coffing2025,coffing2026}. Until recently, these %need to add Coffing et al papers
...
frequency \emph{without} geographic structure pose a different problem. Their persistence
...
Here we use a chromosome-level, annotated reference genome for \illex{} recently made available in \citet{Small2026} to reanalyse
range-wide whole-genome variation at substantially greater genomic resolution. Our
dataset comprises 633 individuals spanning eleven fishery-management divisions \citep{Baker2025}, together
with \coin{} \citep{?COIN} as an outgroup for divergence-based inference and %?COIN needs a reference
...
scan the remaining genome for recent positive selection using complementary methods
\citep{kern2018diploshic,johri2022}. %is johri the correct reference, raisd and sf2
...
males and 291 ZO females, consistent with a ZZ/ZO system \citep{Coffing2025}
(\cref{fig:overview}); Autosomal analyses use both sexes; the Z, on which males are diploid and females
...
full callsets (551{,}344 SNPs in 12{,}007 genes) the empirical differentiation lies entirely within the drift % what is the 551 here? sample size is 633
...
and the inversion is geographically non-clinal. % figure to go w/ the this statement figS6?
...
\paragraph{Reference genome carries derived inversion}
...
\paragraph{The inversion does not follow neutral trajectory.}
...
of an old polymorphism that was once common, not of a recent rise. Two mutation-rate-free diffusion arguments % do we need more detail here?
exclude neutrality. \emph{Attainment}: reaching the inferred historical peak frequency of $\sim$0.74 by drift
takes at minimum $\sim$1.15 million generations at the smallest $\Ne$ --- 1.36$\times$ the entire
...
(Supplementary Fig.~S14). Thus the best fit model supports that the inversion was not lost due to drift alone.
...
The data constrain the \emph{shape} of
...
Fig.~S5). However, with accepted caveat that every absolute age scales inversely with the mutation rate $\mu=3\times10^{-9}$, which may contain rate uncertainty or variation.

\paragraph{Characterizing the inversion.}
It is not a region of locally suppressed crossing-over since recombination inside the body is at the chromosomal
background rate by direct measurement (the $\sim$19~kb LD windows a recombination-map method reads are blind % is this true? FigS12 show chr2 inv and col supressed.
to a 19-Mb barrier; Supplementary Fig.~S12), so the differentiation is a consequence of the arrangement
barrier itself. It is not locally adaptive since there is no cline and geographic $\Fst\approx0$, and the
migration--selection scenario that explains most well-studied inversions \citep{kirkpatrick2006} cannot apply.
And it is not a swept region because the genome-wide scan below finds zero cross-method-concordant sweeps on
chromosome~2. Although, we find that the recombination-map method is broadly elevated and the classifier over-calls across the
inversion, but the two never co-localise into focal concordance, expected for a
balancing-selected supergene rather than a swept locus. These were correctly filtered by the concordance requirement.
...
recombination-suppressed supergene. It is by comparison, gene-poor with only 89 gene models, 45 with cross-species functional
annotation, against a median of $\sim$50 annotated genes in random windows of the same size %how is that gene poor, it is above the median, what percentile?
Overall functional classes are diverse, with lipid metabolism, DNA repair, transcription and splicing, signalling, and no
...
with random gene sets of the same size produce more nominal hits than the
inversion genes (86.6$\pm$60 vs 69; permutation $p=0.51$).Even with a $\sim$40-fold per-term
enrichments, it only depends on as few as two genes (Supplementary Fig.~S21).
...
differences are $\sim$2.3-fold over-represented and clustered.All 19 fall in just nine genes, led by a cytochrome~P450 (\textit{CYP4X1}) and an
acidic-repeat protein (four each) and including a phospholipase~A2, the ciliary gene \textit{RPGRIP1L}, and a
cysteine-rich venom-like protein.The expected architecture of a balancing-selected inversion
that recombines with its homologue only rarely, and the raw material from which a supergene's mutation load
and eventual degeneration are built \citep{jay2021,berdan2023}. % did we test mutational load?

\subsection{Demographic history show and expanding population}\label{sec:demog}
