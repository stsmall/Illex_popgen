...
and the inversion itself is geographically non-clinal (\cref{fig:map}; Supplementary Fig.~S6 and Table~S2).
...
Three further features of the genome set the stage for the sweep scan. Recombination, as estimated by a deep-learning LD-based map, is $\sim$0.20~cM/Mb on the autosomes (male-map median 0.200 raw and 0.213 after bootstrap correction; 0.244 when the male and female maps are averaged; Supplementary Fig.~S12). Because the estimator reads linkage disequilibrium it recovers the sex-averaged rate whichever sex is sampled, so heterochiasmy is not identifiable from it and the maps were built per sex only for historical reasons; the female-only map in fact collapses to a near-constant and is uninformative, and the male map is used throughout. The map is flat with respect to gene content: the rate varies by only $\sim$6\% between windows adjacent to exons and windows $>$100~kb from any exon, unchanged when $\pi$ is held fixed (Supplementary Fig.~S17a), as a crossover rate should be. It is also compressed at chromosome
...
\begin{figure}[p]\centering
  ...
  \caption{\textbf{Sampling and chromosome-2 arrangement frequency.} \illex{} sampled across eleven NAFO
    ...
    $N$). The inverted (BB) frequency is non-clinal (0.29--0.46 across latitude), consistent with geographic $\Fst\approx0$: the inversion is not locally adaptive.}
  \label{fig:map}
\end{figure}
