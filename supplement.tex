...
\begin{figure}[h]\centering
  ...
  \caption{\textbf{Per-chromosome recombination-rate distribution} (ReLERNN). All boxes are raw predictions of the male autosomal network so that chromosomes are compared on the same footing: the 43 scanned autosomes, chromosome~2 split into its collinear flanks and the inverted body (60.54--79.50~Mb), and the Z (male and female runs). Chromosome~2 is an ordinary autosome (median 0.202 vs 0.200~cM/Mb genome-wide, the 60th percentile of per-chromosome medians) and the inverted body is not depressed relative to the flanks (0.2015 vs 0.2023): the estimator reads within-window linkage disequilibrium at the $\sim$19-kb scale and does not register the arrangement-level barrier, which is shown directly by main-text Fig.~3b and Supplementary Figs.~S4 and S5. The female autosomal map collapses to a near-constant (Supplementary Fig.~S22b) and is not shown here.}
...
\begin{figure}[h]\centering
  ...
  \caption{\textbf{The chromosome-1 recombination landscape, and what the LD-based map does not resolve.}
    \textbf{(a)}~Male ReLERNN estimate in 15~kb windows with 95\% bootstrap CIs and a 0.6-Mb rolling median
    (median 0.209~cM/Mb). The shaded 2.5-Mb block at 45.0--47.5~Mb is 80\% repeat sequence (vs 52\%
    chromosome-wide), is almost entirely masked, and carries no ReLERNN windows: the centromere. In the
    accessible pericentromeric flanks the estimate dips only $\sim$5\%, and across the whole 103~Mb the rolling
    median spans just 0.17--0.23~cM/Mb ($-20\%$ to $+7\%$ of the median) --- the map's chromosome-scale
    dynamic range is compressed, so centromeric suppression cannot be resolved and the ``nearly uniform''
    landscape is in part an estimator ceiling. \textbf{(b)}~The female-only estimate collapses to a
    near-constant (IQR 0.293--0.298~cM/Mb) and is uninformative; the sex-averaged mean is what the estimator
    recovers (see main text). \textbf{(c)}~Nucleotide diversity $\pi$ in 10~kb windows halves on the flank of
    the masked block --- the pericentromeric signature that the recombination estimate in (a) does not show.
    The block of tripled $\pi$ at 24--31~Mb (hatched) is one of 22 such $\geq$1-Mb blocks genome-wide. It is
    not a structural polymorphism: a local PCA shows no karyotype clusters (its Gaussian-mixture ``clusters''
    track coverage, and heterozygosity is flat across them, unlike the chr2 inversion), there is no excess
    heterozygosity, no loss to the excess-heterozygosity filter, and the per-individual read-depth ratio to
    flanking sequence is tight and unimodal (median 0.95, IQR 0.94--0.97; no duplication carriers); its
    folded allele-frequency spectrum is identical to the flanks. It is gene-poor (coding fraction 0.001), only
    35\% alignable to \coin{} (vs 87--89\% in flanks) and 1.65-fold more divergent where alignable
    (3.3\% vs 1.9--2.1\%), and low-coverage \textit{I.\ argentinus} reads call $\sim$3$\times$ more variants
    in it --- a fast-evolving, largely lineage-specific gene desert (elevated mutation input plus relaxed
    linked selection), not an inversion or introgression.}
