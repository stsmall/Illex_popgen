...
\begin{table}[h]\centering
  \caption{\textbf{Tier-2 (cross-method-concordant) sweep genes.} The 34 regions where the diploSHIC-HMM
    sweep signal is independently corroborated by both RAiSD and SweepFinder2 (33 auto-detected plus the
    chr43 balancing locus handled separately). Seven regions carry a named protein-coding gene (12 genes,
    listed); the remaining concordant regions are gene deserts or uncharacterised loci. Coordinates in Mb;
    $\alpha$ and DoS from the McDonald--Kreitman test against \textit{I.~coindetii} (`--' where the test is
    unpowered, i.e.\ too few fixed/polymorphic sites). The full candidate set --- all 216 windows, the 34
    concordant regions, and the background-selection test --- is provided as machine-readable tables
    (\texttt{outlier\_scan/\{windows,regions,concordant33\_genes\_ENTAP\_GO,concordant33\_genes\_MK,%
    concordant33\_BGS\_test\}.tsv}).}
