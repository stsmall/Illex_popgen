...
\begin{figure}[h]\centering
  ...
  \caption{\textbf{Per-chromosome recombination-rate distribution} (ReLERNN). All boxes are raw ReLERNN predictions from a single network so that chromosomes are compared on the same footing. They cover the 43 scanned autosomes, chromosome~2 split into its collinear flanks and the inverted body (60.54--79.50~Mb), and the Z. Chromosome~2 is an ordinary autosome (median 0.202 vs 0.200~cM/Mb genome-wide, the 60th percentile of per-chromosome medians) and the inverted body is not depressed relative to the flanks (0.2015 vs 0.2023), because the estimator reads within-window linkage disequilibrium at the $\sim$19-kb scale and does not register the arrangement-level barrier, which is shown directly by main-text Fig.~3b and Supplementary Figs.~S4 and S5. }
