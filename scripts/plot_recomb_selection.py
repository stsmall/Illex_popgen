...
fig, axs = plt.subplots(1, 3, figsize=(15.0, 4.6), gridspec_kw={"wspace": 0.36})

# (a) recomb vs distance from exon -- the CONTROL. Drawn on the full recombination-rate
#     axis so the near-flatness is visible: the whole gradient is
...
axs[0].fill_between(mids, m - se, m + se, color=GREEN, alpha=0.2, lw=0)
axs[0].plot(mids, m, color=GREEN, lw=2, marker="o", ms=3)
axs[0].set_ylim(0, max(0.32, r[np.isfinite(r)].mean() * 1.5))
axs[0].set_xlabel(r"$\log_{10}$ distance to nearest exon (bp)")
axs[0].set_ylabel("recombination rate (cM/Mb)")
axs[0].set_title(f"a   recombination flat vs gene proximity ($\\rho={rho_rd:+.2f}$)",
                    loc="left", fontweight="bold", fontsize=10)
axs[0].text(0.04, 0.96,
               ...
               transform=axs[0].transAxes, fontsize=7.6, color=GREY, va="top", ha="left")
despine(axs[0])
...
axs[1].fill_between(mids, m - se, m + se, color=BLUE, alpha=0.2, lw=0)
axs[1].plot(mids, m, color=BLUE, lw=2, marker="o", ms=3)
axs[1].set_xlabel(r"$\log_{10}$ distance to nearest exon (bp)")
axs[1].set_ylabel(r"nucleotide diversity $\pi$")
axs[1].set_title(f"b   $\\pi$ rises away from genes ($\\rho={rho_pd:+.2f}$)",
                    loc="left", fontweight="bold", fontsize=10)
axs[1].text(0.04, 0.96, "$\\pi$ recovers with distance from coding\nsequence while recombination (a) does not —\nthe footprint of linked selection",
               transform=axs[1].transAxes, fontsize=7.6, color=GREY, va="top", ha="left")
despine(axs[1])
...
axs[2].fill_between(mids, m - se, m + se, color=BLUE, alpha=0.2, lw=0)
axs[2].plot(mids, m, color=BLUE, lw=2, marker="o", ms=3)
axs[2].set_xlabel("local coding density (fraction CDS)")
axs[2].set_ylabel(r"nucleotide diversity $\pi$")
axs[2].set_title(f"c   $\\pi$ falls in gene-dense regions ($\\rho={rho_cds:+.2f}$)",
                    loc="left", fontweight="bold")
axs[2].text(0.5, 0.94, "background selection reduces $\\pi$ where\ngenes are dense $\\Rightarrow$ locally reduced $N_e$",
               transform=axs[2].transAxes, fontsize=7.6, color=GREY, va="top", ha="left")
despine(axs[2])
